    ...
    try:
        ...
        for attempt in range(3):  # 最多重试3次
            ...
            if "个人信息" in driver.page_source:
                ...
                # 手动输入验证码
                if manual_captcha_input(driver):
                    logging.info(f"{account['username']} 手动输入验证码后登录成功")
                    return True
                else:
                    logging.warning(f"{account['username']} 手动输入验证码后仍然失败，重试自动识别验证码")
...
def attempt_booking(driver, target_times):
    selected_seats = []
    failed_times = []

    # 遍历每个目标时间段，查找并选择可预订的场地
    for target_time in target_times:
        logging.info(f"尝试选择时间段 {target_time} 的场地...")
        if find_and_click_first_available_seat_for_time(driver, target_time):
            logging.info(f"成功选择了时间段 {target_time} 的场地。")
            selected_seats.append(target_time)
        else:
            logging.info(f"在时间段 {target_time} 未找到可预订的场地。")
            failed_times.append(target_time)

    # 如果选择了任何场地，则尝试提交订单
    if selected_seats:
        logging.info(f"已选择时间段 {selected_seats} 的场地，准备提交订单...")
        if click_order_button(driver):
            logging.info(f"订单已成功提交。选择了以下时间段的场地: {selected_seats}")
            if failed_times:
                logging.info(f"以下时间段未能找到可预订的场地: {failed_times}")
            return True
        else:
            logging.error("提交订单失败，选择的场地未成功预订。")
            return False
    else:
        logging.info(f"在所有时间段 {target_times} 内未找到可预订的场地。")
        return False


def click_order_button(driver):
    try:
        # 假设“立即下单”按钮的 XPath 已知
        order_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//button[text()='立即下单']"))
        )
        order_button.click()
        logging.info("立即下单按钮已点击。")
        return True
    except TimeoutException:
        logging.error("未能在规定时间内找到‘立即下单’按钮。")
        return False
...
def run_booking_process(driver, target_times, weekday):
    select_next_wednesday_date(driver, weekday)
    return attempt_booking(driver, target_times)
...
def main(account, court_type, venue_name, target_times, weekday):
    driver = setup_driver()
    login_successful = login(account, driver)
    if login_successful:
        select_court_type(driver, court_type)
        time.sleep(3)  # 等待页面加载
        # 等等抢票时机，根据参数配置，等待一段时间，当时间一致立马进入抢票
        select_venue(driver, venue_name)
        # time.sleep(3)  # 等待页面加载
        logging.info("场地预定窗口已打开，准备开始抢票")
        if run_booking_process(driver, target_times, weekday):
            logging.info("抢票成功，请及时支付订单")


def schedule_booking(accounts, court_type, venue_name, target_times, weekday):
    with ThreadPoolExecutor(max_workers=min(3, len(accounts))) as executor:
        for account in accounts:
            executor.submit(main, account, court_type, venue_name, target_times, weekday)
...
if __name__ == '__main__':
    ...
    # param weekday: 目标星期几，1=周一, 2=周二, ..., 7=周日
    schedule_booking(accounts, "网球", "东区网球场", target_times, "3")
